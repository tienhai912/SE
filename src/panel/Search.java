...
public class Search extends Panel {
	...
	private ArrayList<Integer> infoLineStationsId;
	private ArrayList<stationInNavi> stationsInNavi;
	...
	public void createInfoTextField() {
		...
		infoTextField.setPlaceholder("Station/Line");
	...
	public void createFromTextField() {
		...
		fromTextField.setPlaceholder("Starting Place");
	...
	public void createToTextField() {
		...
		toTextField.setPlaceholder("Destination");
	...
	public void createChoiceBox() {
		// choiceBox
		String list[] = { "Shortest Road", "Least Transfers" };
		JComboBox<String> choiceBox = new JComboBox<String>(list);
		choiceBox.setPreferredSize(new Dimension(120, 30));
		...
		naviPanel.add(Box.createHorizontalStrut(35));
	}
	...
	public void infoShowStationResult() {
		...
		result = client.handleRequestArrivalTime(
				client.handleRequest("http://server/stations/" + infoStation.getId() + "/arrivaltimeinfo"));
		result.trim();
	...
	public void infoShowLineResult() {
		...
		Client client = new Client();
		result = client.handleRequestLineId(client.handleRequest("http://server/line/" + infoLine.getId()),
				infoLineStationsId);
		result.trim();
		resultArray = result.split("\\r\\n|\\n|\\r");
	...
	public void naviShowResult() {
		if (fromStation != null && toStation != null) {
			...
			// handle
			Client client = new Client();
			stationsInNavi = new ArrayList<stationInNavi>();
			if (shortestRoad) {
				result = client.handleRequestNavi(
						client.handleRequest(
								"http://server/nav/" + fromStation.getId() + "/" + toStation.getId() + "/shortest"),
						stationsInNavi);
			} else {
				result = client.handleRequestNavi(client.handleRequest(
						"http://server/nav/" + fromStation.getId() + "/" + toStation.getId() + "/leasttransfer"),
						stationsInNavi);
			}
			result.trim();
			resultArray = result.split("\\r\\n|\\n|\\r");
			naviStations = new ArrayList<Station>();
			for (stationInNavi i : stationsInNavi) {
				naviStations.add((Station) display.getIdStations().get(i.getStationId()));
			}
			...
			try {
				...
				for (int i = 0; i < stationsInNavi.size(); i++) {
					stationInNavi temp = stationsInNavi.get(i);
					if (temp.isChangeLine()) {						
						naviDoc.insertString(naviDoc.getLength(), "\nChange at " + naviStations.get(i).getName() + "\n",
								null);
					}
				}
				naviDoc.insertString(naviDoc.getLength(), "\n", null);
				...
				stationWordStyle(naviDoc);
				naviDoc.insertString(naviDoc.getLength(), "\n" + resultArray[0] + " km\n", null);
				naviDoc.insertString(naviDoc.getLength(), resultArray[1] + " VND\n", null);
